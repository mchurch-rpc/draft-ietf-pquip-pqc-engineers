...
normative:
  ...
  Grovers:
    ...
    date: 1996-07-01
    refcontent: >
      STOC '96: Proceedings of the twenty-eighth annual ACM symposium on Theory of Computing, pp. 212-219
    seriesinfo:
...
informative:
  ...
  LattFail1:
     ...
     target: https://link.springer.com/chapter/10.1007/978-3-030-17259-6_19
     author:
       -
     ...
     date: 2019-04-06
     refcontent: Public-Key Cryptography – PKC 2019, Lecture Notes in Computer Science, vol. 11443, pp. 565-598
     seriesinfo:
  ...
  LattFail2:
     ...
     refcontent: Advances in Cryptology – EUROCRYPT 2020, Lecture Notes in Computer Science, vol. 12107, pp. 3-33
     seriesinfo:
  ...
  SP-1800-38C:
      title: "Migration to Post-Quantum Cryptography Quantum Readiness: Testing Draft Standards, Volume C: Quantum-Resistant Cryptography Technology Interoperability and Performance Report"
  ...
  BIKE:
    ...
    date: false
  PQUIP-WG:
    title: "Post-Quantum Use In Protocols (pquip)"
    author:
      -
        org: IETF
  ...
  OQS:
    ...
    date: false
  CRQCThreat:
    title: "Landscape of Quantum Computing"
  ...
  AddSig:
    title: "Post-Quantum Cryptography: Additional Digital Signature Schemes"
  ...
  BPQS:
    ...
    target: https://eprint.iacr.org/2018/658
    author:
      -
...
<!-- [rfced] Reference Review Notes/AQs:
* REFs
...
* Note to PE: Should this I-D be replaced by the more current version?
...
* Note to PE: This I-D may have been published as an RFC by the time you see this
note. *
...
* Note to PE: authors names are incorrect; see note in FYI AQ *
[I-D.ietf-pquip-pqc-hsm-constrained]
draft-ietf-pquip-pqc-hsm-constrained-02
IESG State: I-D Exists as of 11/26/25

[I-D.ietf-pquip-pqt-hybrid-terminology] -> [RFC9794]
Updated to RFC 9794
...
** Correct URL for [BIKE]

Please review. The URL for [BIKE] points to the same page for [HQC] with a title
of "HQC". Is this the intended URL for this reference? Note: we found the
following home page for BIKE (Bit Flipping Key Encapsulation)
: https://bikesuite.org/.
...
** General AQ for updates to the references section

Note: We updated the references in the references section to include titles,
URLs, and additional publication information that may be helpful for future
readers. Please review and let us know if you have any concerns or corrections.

-->

<!-- [rfced] FYI - We will do the following when we convert the file to RFCXML:

- Note to PE: [I-D.ietf-pquip-pqc-hsm-constrained] Authors names are incorrect.
...
Recommend doing the following update in the XML:
...
<!-- [rfced] Note to PE:
The following may require clarification from the authors:
...
"CNSA 2.0" is a suite of algorithms from the NSA, not an organization. The
organization is the National Security Agency (NSA).
...
<!-- [rfced] Note to PE:
It may be helpful to the reader to note that FIPS 206 is still in draft form.
This is mentioned in the [NISTFINAL] reference.

Current:
   [FN-DSA]: FN-DSA is a lattice signature scheme (FIPS 206) (Section 8.1 and Section 10.2).
-->
...
<!-- [rfced] Note to PE (hyphenation):
[FrodoKEM] uses "key-encapsulation" rather than "key encapsulation".

Current:
    [FrodoKEM]: Key Encapsulation mechanism based on the hardness of learning
    with errors in algebraically unstructured lattices.
-->
...
<!-- [rfced] Note to PE: (capitalization)
The authors are using mixed versions of the expansion for HPKE:

Current:
   One widely deployed scheme that achieves this is HPKE (Hybrid Public Key
   Encryption) [RFC9180].HPKE (hybrid public key encryption) [RFC9180] works with a
   combination of KEMs, KDFs and AEAD (authenticated encryption with additional
   data) schemes.
-->
...
HPKE (hybrid public key encryption) {{RFC9180}} works with a combination of KEMs, KDFs and AEAD (authenticated encryption with additional data) schemes. HPKE includes three authenticated variants, including one that authenticates possession of a pre-shared key and two optional ones that authenticate possession of a key encapsulation mechanism (KEM) private key. HPKE can be extended to support hybrid post-quantum KEM {{?I-D.ietf-hpke-pq}}. ML-KEM does not support the static-ephemeral key exchange that allows HPKE based on DH based KEMs and its optional authenticated modes as discussed in {{Section 1.5 of ?I-D.connolly-cfrg-xwing-kem}}.
...
ML-DSA offers both deterministic and randomized signing and is instantiated with 3 parameter sets providing different security levels. Security properties of ML-DSA are discussed in {{Section 9 of !RFC9881}}.
...
During the transition from traditional to post-quantum algorithms, there may be a desire or a requirement for protocols that use both algorithm types. {{?RFC9794}} defines the terminology for the post-quantum and traditional (PQ/T) hybrid schemes.
...
At least one scheme has been proposed that allows the pair of certificates to exist as a single certificate when being issued and managed, but dynamically split into individual certificates when needed ({{?I-D.bonnell-lamps-chameleon-certs}}).
